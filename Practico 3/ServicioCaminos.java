...
public class ServicioCaminos {
    ...
    private void camino(int vo, int vd, int contador) {
        ...
        // No usamos el contador porque se puede contar los arcos usando la longitud del
        // camino con camino.size()
        while (ady.hasNext() && camino.size() <= lim) {
            Integer ad = ady.next();

            // Obtengo el arco y controlo con el if que no este visitado y lo agrego
            Arco<?> arco = grafo.obtenerArco(vo, ad);
            if (!arcosVisitados.contains(arco)) {
                arcosVisitados.add(arco); // Agrego el arco a arcosVisitados
                camino.add(ad); // Agrego el adyacente si el arco no esta visitado por eso lo pongo aca
